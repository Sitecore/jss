...
export function generateConfig(configOverrides?: { [key: string]: string }): void {
  ...
  computedConfig.graphQLEndpoint =
    '`${config.sitecoreApiHost}${config.graphQLEndpointPath}?sc_apikey=${config.sitecoreApiKey}`';

  // Create an empty object defining all our configuration properties, so module export has proper typing
  const configProps = Object.assign({}, config, computedConfig);
  Object.keys(configProps).forEach((prop) => (configProps[prop] = ''));
  let configText = `/* eslint-disable */
// Do not edit this file, it is auto-generated at build time!
// See scripts/bootstrap.ts to modify the generation of this file.
var config = ${JSON.stringify(configProps, null, 2)};\n`;

  // Set base configuration values, allowing override with environment variables
  Object.keys(config).forEach((prop) => {
    configText += `config.${prop} = process.env.${prop} || "${config[prop]}",\n`;
  });
  // Set computed values
