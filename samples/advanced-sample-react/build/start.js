...
server.use("/data/context", authenticationMock);
...
function authenticationMock(req, res, next) {
  if (req.cookies.loggedIn) {
    //mock a logged in user if we have the cookie
    const context = getJsonOrYaml(path.join(process.cwd(), req.path));
    context.user = {
      user: "extranet",
      name: "lorem"
    };
...
function sscAuthMockBefore(app) {
  console.log("before");
