...
const MainNav = ({
    ...
    <Navbar.Header>
      ...
      <Navbar.Brand>
        <a
          ...
          href={getRouteUrl(currentLang, '/')}
          onClick={(e) => handleNavItemClick(e, props)}
        >
          {t('Start Bootstrap')}
        </a>
      </Navbar.Brand>
        ...
        <Nav navbar pullRight>
            ...
            if (!navItem.children) {
              return (
                <NavItem
                  key={index}
                  className="page-scroll"
                  href={getRouteUrl(currentLang, navItem.path)}
                  onClick={(e) => handleNavItemClick(e, props)}
          ...
          >
            <MenuItem
              ...
              href={getRouteUrl('en', currentRoute)}
              onClick={(e) => handleNavItemClick(e, props)}
            >
              English (US)
            </MenuItem>
            <MenuItem
              key="es-mx"
              className="flag-icon flag-icon-mx"
              href={getRouteUrl('es-mx', currentRoute)}
              onClick={(e) => handleNavItemClick(e, props)}
            >
              Español (MX)
            </MenuItem>
          ...
          {user ? (
            <NavItem
              key="logout"
              className="nav-btn"
              onClick={(e) => props.actions.logout(getRouteUrl(currentLang, currentRoute))}
            >
              {t('Logout')}
            </NavItem>
          ) : (
            <NavItem key="login" className="nav-btn" onClick={(e) => props.actions.showLoginForm()}>
              {t('Login')}
            </NavItem>
          )}
        </Nav>
  ...
  <LoginModal
    ...
    currentRoute={getRouteUrl(currentLang, currentRoute)}
    onHide={props.actions.hideLoginForm}
