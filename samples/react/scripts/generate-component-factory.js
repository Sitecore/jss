...
function getComponents(folderPath) {
  fs.readdirSync(folderPath).forEach((componentFolder) => {
    const componentFolderFullPath = path.join(folderPath, componentFolder);
    ...
    if (
      ...
      var importPath = componentFolderFullPath.split('\\').join('/').replace('src', '..');
...
function generateComponentFactory() {
  ...
  getComponents(componentRootPath);
