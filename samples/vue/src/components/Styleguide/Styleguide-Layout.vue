...
<template>
  <div class="row">
    <div class="col-sm-8 col-lg-10">
      <sc-placeholder name="JssVueWeb-jss-styleguide-layout" :rendering="rendering" />
    </div>
...
export default {
  ...
  computed: {
    sections() {
      ...
      return getChildPlaceholder(this.rendering, 'JssVueWeb-jss-styleguide-layout')
        .filter((section) => getFieldValue(section, 'heading'))
        .map((section) => ({
          heading: getFieldValue(section, 'heading'),
          id: `i${section.uid.replace(/[{}]/g, '')}`,
          children: getChildPlaceholder(section, 'JssVueWeb-jss-styleguide-section')
