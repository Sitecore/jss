...
| `serverSideRenderingEngineApplicationUrl` | Only relevant when `http` rendering engine is enabled. Defines the URL that will be used when creating absolute links in the rendered HTML. Link replacement is necessary in order for relative links in the rendered HTML to be able to load in the Experience Editor. If not specified (default), link replacement will not be performed.<br/><br/>Default: `` |
