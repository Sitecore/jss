...
2. Add [environment variables](https://nextjs.org/docs/basic-features/environment-variables#environment-variables-on-vercel) to Vercel or modify `.env` file. We recommend to use environment variables in Vercel:
	...
	* `DECISIONS_ENDPOINT` - your Sitecore decisions API endpoint. The `DECISIONS_ENDPOINT` is required for personalizing using Sitecore. By default, the endpoint is `decisionsEndpointPath` defined in your `package.json` which means it is proxied through Node instance to `sitecoreApiHost`.
