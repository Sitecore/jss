...
2. Add [environment variables](https://nextjs.org/docs/basic-features/environment-variables#environment-variables-on-vercel) to Vercel or modify `.env` file. We recommend to use environment variables in Vercel:
	...
	* `SITECORE_SERVICES_API_KEY` - your SSC API Key which has been configured for use with the Tracking Service (Sitecore.Analytics.Controllers.TrackController, Sitecore.Analytics) and Personalization Service controllers (Sitecore.LayoutService.Mvc.Controllers.LayoutServiceController, Sitecore.LayoutService.Mvc). This key is required when using Sitecore Experience Edge and personalization/tracking powered by Sitecore Content Delivery instance.
	* `IS_TRACKING_ENABLED` - determines whether tracking is enabled or not. If not specified tracking is enabled.
