...
2. The next step varies, depending on your workflow. If you are working Code First, then add the new root placeholder names to the `config.rootPlaceholders` array in `package.json`. In you are working Sitecore First, add the new root placeholders to the layout definition item in Sitecore.
...
If you don’t like the project structure generated by `jss scaffold`, you can customize `scripts/scaffold-component.ts`. If you are using Sitecore First workflow, `scripts/scaffold-component.ts` can be updated to stop creating file under `sitecore/definitions` since they are not needed.
