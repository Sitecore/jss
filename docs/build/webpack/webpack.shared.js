...
export default function(envVars) {
  const defaultEnv = {
    ...
    production: true, // when true sets node env to prod (production react) and enables uglify
    ...
    devserver: true, // whether to use webpack-dev-server or not (note: only used by client bundle)
  };
...
function getGlobalVariables(envVars) {
  ...
  if (envVars.content === 'disconnected' && envVars.devserver) {
    ...
    globalVars.__SC_API_HOST__ = JSON.stringify(`https://localhost:${jssConfig.devServerPort}`);
  }
