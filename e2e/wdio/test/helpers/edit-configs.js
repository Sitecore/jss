...
class EditConfigs {
  ...
  editWebConfig(filepath, serverSideRenderingEndpointUrl) {
    ...
    let serverSideRenderingEndpoint = webConfig.match(/(?<=serverSideRenderingEngineEndpointUrl=")(.*)(?=")/g)[0]
    console.log(serverSideRenderingEndpoint)
  ...
  editDotEnv({filepath, jssEditingSecret, apiKey, graphQLEndpoint, rootItemPath}={}) {
    ...
    dotEnv = dotEnv.replace(`JSS_EDITING_SECRET=${currentJssEditingSecret}`, `JSS_EDITING_SECRET=${jssEditingSecret}`)
    ...
    dotEnv = dotEnv.replace(`GRAPH_QL_ENDPOINT=${currentGraphQLEndpoint}`, `GRAPH_QL_ENDPOINT=${graphQLEndpoint}`)
    let currentRootItemPath = dotEnv.match(/(?<=ROOT_ITEM_PATH=)(.*)(?=)/g)
    if (currentRootItemPath) {
      dotEnv = dotEnv.replace(`ROOT_ITEM_PATH=${currentRootItemPath}`, `ROOT_ITEM_PATH=${rootItemPath}`)
