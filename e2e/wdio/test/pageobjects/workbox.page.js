...
class WorkboxPage extends Page {
    ...
    async publishContentItems() {
        ...
        await browser.switchToFrame(await $('[src="/sitecore/shell/default.aspx?xmlcontrol=Workbox&mo=preview"]'))
        const jssDevelopmentWorkflowCheckbox = await ( await $('label=JSS Development Workflow')).$('..//input')
        ...
        if(numOfDevItems.match(/\(([^)]+)\ /)) {
          ...
          if (parseInt(numOfDevItems.match(/\(([^)]+)\ /)[1]) > 0) {
            if (await (await (await developmentModeCollapsed[0]).$('..')).getAttribute('class') === 'scCollapsed') {
        ...
        if(numOfContentItems.match(/\(([^)]+)\ /)) {
            if (parseInt(numOfContentItems.match(/\(([^)]+)\ /)[1]) > 0) {
              if(await (await (await developmentModeCollapsed[1]).$('..')).getAttribute('class') === 'scCollapsed') {
