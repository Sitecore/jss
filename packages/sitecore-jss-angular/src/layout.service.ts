...
import { dataApi, LayoutServiceData, LayoutServiceRequestOptions, PlaceholderData } from '@sitecore-jss/sitecore-jss';
...
export class LayoutService {
  ...
  getRouteData(route: string, options: LayoutServiceRequestOptions<LayoutServiceData>): Observable<LayoutServiceData | LayoutServiceError> {
    return fromPromise(fetchRouteData(route, options))
      .pipe(catchError(LayoutService.getLayoutServiceError));
  }
  ...
  getPlaceholderData(
    ...
    return fromPromise(fetchPlaceholderData(placeholderName, route, options))
      .pipe(catchError(LayoutService.getLayoutServiceError));
  }
}
