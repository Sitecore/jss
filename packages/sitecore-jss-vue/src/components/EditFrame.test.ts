...
describe('<EditFrame />', () => {
  it('should render nothing when not in pageEditing mode', () => {
    const rendered = mount(EditFrame, {props: {
  ...
  it('should render a frame when in pageEditing mode', () => {
    ...
    expect(rendered.html()).toBe('<div class=\"scLooseFrameZone\"><span class=\"scChromeData\">{}</span></div>');
  });

  it('should render child content within a frame when in pageEditing mode', () => {
    const rendered = mount(EditFrameWithText, {props: {
      context: editingContext,
    }});
  ...
  it('should render the datasource', () => {
    ...
    expect(editSpan.text().includes('"contextItemUri":"sitecore://master/testItemId?lang=en"')).toBe(true);
  });
