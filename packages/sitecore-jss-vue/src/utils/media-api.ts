...
export const updateImageUrl = (
  ...
  const parsed = new URL(replaceMediaUrlPrefix(url, mediaUrlPrefix));

  const qs = Object.fromEntries(parsed.searchParams.entries());
  const requiredParams = getRequiredParams(qs);
  ...
  const queryOverride = new URLSearchParams(query as Record<string, string>);
  parsed.search = queryOverride.toString();
