...
describe('<FileUpload />', () => {
  ...
  it('should check if validator is enabled', () => {
    ...
    expect(inst.isValidatorEnabled('yyy')).to.be.true;
    expect(inst.isValidatorEnabled('zzz')).to.be.true;
    expect(inst.isValidatorEnabled('xxx')).to.be.true;
    expect(inst.isValidatorEnabled('ggg')).to.be.false;
  });
});
