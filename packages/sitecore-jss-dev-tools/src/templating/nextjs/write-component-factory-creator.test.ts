...
describe('writeComponentFactoryCreator', () => {
  ...
  describe('writeComponentFactory', () => {
    ...
    it('should write component factory to file', () => {
      ...
      const actualDestPath = writeFileStub.getCall(0).args[0];
      const actualOutput = writeFileStub.getCall(0).args[1];      
      expect(actualDestPath).to.be.equal('src/temp/componentFactoryCreator.ts');
  ...
  describe('writeProjectComponents', () => {
    it('should write component factory file for project', () => {
      ...
      const actualDestPath = writeFileStub.getCall(0).args[0];
      const actualOutput = writeFileStub.getCall(0).args[1];      
      expect(actualDestPath).to.be.equal('src/temp/projects/manhattan.ts');
      expect(actualOutput).to.be.equal(expectedOutput);
    });
  });
