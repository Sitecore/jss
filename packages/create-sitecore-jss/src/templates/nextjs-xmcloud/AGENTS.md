...
- **Where:** `src/pages/api/editing/render.ts`
- **render.ts:** Handles POST requests from the Sitecore Experience Editor (chromes mode). Uses component map and layout data.
- **Do not:** Remove or alter this route; the Experience Editor depends on it.
...
- **Quick checks:** If FEAAS components do not render, verify component-builder plugins include feaas. Note: BYOC/FEAAS component events do not fire in this app — CloudSDK is not initialized (base no-op Bootstrap is used).
...
- **Sitecore patterns:** Preserve FEAAS and BYOC integration. When adding components, register in the appropriate plugin (component-builder, BYOC, FEAAS). Keep the editing API route in sync.
...
- **Follow existing patterns:** Keep the editing route in sync.
