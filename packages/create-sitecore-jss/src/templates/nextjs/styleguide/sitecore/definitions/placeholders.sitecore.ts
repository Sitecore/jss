...
export default function addPlaceholdersToManifest(manifest: Manifest): void {
  manifest.addPlaceholder(
    { name: '<%- appPrefix ? `${helper.getPascalCaseName(appName)}-` : "" %>jss-main', displayName: 'Main' },
    ...
    { name: '<%- appPrefix ? `${helper.getPascalCaseName(appName)}-` : "" %>jss-tabs', displayName: 'Tabs', id: 'tabs-placeholder' }
  );
}
