...
export default function StyleguideExplanatoryComponent(manifest: Manifest): void {
  manifest.addTemplate({
    name: '<%- appPrefix ? `${helper.getPascalCaseName(appName)}-` : "" %>Styleguide-Explanatory-Component',
    id: '<%- appPrefix ? `${helper.getPascalCaseName(appName)}-` : "" %>styleguide-explanatory-component-template',
    fields: [
