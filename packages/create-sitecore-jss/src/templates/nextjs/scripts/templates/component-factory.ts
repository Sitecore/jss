...
export function componentModule(componentName: string) {
  const siteKey = \`SITE_\${config.jssAppName}_\${componentName}\`;
  const themeKey = \`THEME_\${config.jssAppTheme}_\${componentName}\`;
  const sharedKey = \`SHARED_\${componentName}\`;
  const component = components.has(siteKey) ? components.get(siteKey) :
    components.has(themeKey) ? components.get(themeKey) :
    components.get(sharedKey);
